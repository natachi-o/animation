...
function createLights() {
  ...
  shadowLight.shadow.mapSize.width = 2048; // Increase shadow map size for better quality
...
function createFloor(){
  floor = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({
      color: 0xebe5e7, // Set the base color
      transparent: true, // Enable transparency
      opacity: 0.5      // Set the desired opacity (0.0 is fully transparent, 1.0 is fully opaque)
    })
  );
  floor.rotation.x = -Math.PI / 2;
  floor.position.y = -100;
  floor.receiveShadow = true; // Ensure the floor still receives shadows
  scene.add(floor);
}
...
// Ensure Fan class is defined before it's used
class Fan {
    constructor() {
...
class Lion {
    constructor() {
        ...
        const verticesIndices = [0, 1, 2, 3, 4, 10];

        for (var i = 0; i < verticesIndices.length; i++) {
            const idx = verticesIndices[i] * 3;
            position.array[idx + 2] = 70; // Set z coordinate
            this.bodyInitPositions.push({ x: position.array[idx], y: position.array[idx + 1], z: position.array[idx + 2] });
        }
...
this.leftIris.position.x = 42;
    ...
    look(xTarget, yTarget) {
        ...
        for (let i = 0; i < this.bodyVertices.length; i++) {
            const idx = this.bodyVertices[i] * 3;
            position.setX(idx / 3, this.bodyInitPositions[i].x + this.head.position.x);
            position.setY(idx / 3, this.bodyInitPositions[i].y);
            position.setZ(idx / 3, this.bodyInitPositions[i].z);
        }
        position.needsUpdate = true;
    }
